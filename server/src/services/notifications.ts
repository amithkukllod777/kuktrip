    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr><td style="padding: 20px 32px; background: #f9fafb; border-top: 1px solid #f3f4f6; text-align: center;">
          ...
          <p style="margin: 0; font-size: 10px; color: #d1d5db;">${s.madeWith} <span style="color: #ef4444;">&hearts;</span> &middot; <a href="https://kuklabs.com" style="color: #9ca3af; text-decoration: none;">Part of the Kuklabs ecosystem</a></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtpCfg.from,
      to,
      subject: `Kuk Trip — ${strings.subject}`,
...
export function buildWebhookBody(url: string, payload: { event: string; title: string; body: string; tripName?: string; link?: string }): string {
  ...
  if (isDiscord) {
    return JSON.stringify({
      embeds: [{
        ...
        url: payload.link,
        color: 0x3b82f6,
        footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'Kuk Trip' },
  ...
  if (isSlack) {
    ...
    const link = payload.link ? `\n<${payload.link}|Open in Kuk Trip>` : '';
    return JSON.stringify({
      text: `*${payload.title}*\n${payload.body}${trip}${link}`,
    });
  }

  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'Kuk Trip' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await transporter.sendMail({
      from: config.from,
      to,
      subject: 'Kuk Trip — Test Notification',
      text: 'This is a test email from Kuk Trip. If you received this, your SMTP configuration is working correctly.',
    });
    return { success: true };
  } catch (err) {
...
export async function testWebhook(url: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook(url, { event: 'test', title: 'Test Notification', body: 'This is a test webhook from Kuk Trip. If you received this, your webhook configuration is working correctly.' });
...
export async function testNtfy(cfg: { topic: string; server?: string | null; token?: string | null }): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const sent = await sendNtfy(url, cfg.token ?? null, {
      ...
      body: 'This is a test notification from Kuk Trip. If you received this, your ntfy configuration is working correctly.',
    });
