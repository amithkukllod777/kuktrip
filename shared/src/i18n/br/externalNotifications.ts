...
const br: NotificationLocale = {
  email: {
    footer: 'Você recebeu isso porque tem as notificações ativadas no Kuk Trip.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} convidou você para fundir planos de férias. Abra o Kuk Trip para aceitar ou recusar.`,
    }),
    collection_invite: (p) => ({
      title: 'Convite para coleção',
      body: `${p.actor} convidou você para compartilhar uma coleção. Abra o Kuk Trip para aceitar ou recusar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nova versão do Kuk Trip disponível',
      body: `O Kuk Trip ${p.version} está disponível. Acesse o painel de administração para atualizar.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Olá',
    body: 'Recebemos um pedido para redefinir a senha da sua conta Kuk Trip. Clique no botão abaixo para definir uma nova senha.',
