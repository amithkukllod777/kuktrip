...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in Kuk Trip.',
    manage: 'Manage preferences in Settings',
    madeWith: 'Made with',
    openTrek: 'Open Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open Kuk Trip to accept or decline.`,
    }),
    collection_invite: (p) => ({
      title: 'Collection invite',
      body: `${p.actor} invited you to share a collection. Open Kuk Trip to accept or decline.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'New Kuk Trip version available',
      body: `Kuk Trip ${p.version} is now available. Visit the admin panel to update.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your Kuk Trip account. Click the button below to set a new password.',
