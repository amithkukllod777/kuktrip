...
const gr: NotificationLocale = {
  email: {
    footer: 'Λάβατε αυτό το μήνυμα επειδή έχετε ενεργοποιήσει τις ειδοποιήσεις στο Kuk Trip.',
    manage: 'Διαχείριση προτιμήσεων στις Ρυθμίσεις',
    madeWith: 'Δημιουργήθηκε με',
    openTrek: 'Άνοιγμα Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `Ο/Η ${p.actor} σας προσκάλεσε να συγχωνεύσετε τα σχέδια διακοπών σας. Ανοίξτε το Kuk Trip για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    collection_invite: (p) => ({
      title: 'Πρόσκληση σε συλλογή',
      body: `Ο/Η ${p.actor} σας προσκάλεσε να μοιραστείτε μια συλλογή. Ανοίξτε το Kuk Trip για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Νέα έκδοση Kuk Trip διαθέσιμη',
      body: `Η έκδοση Kuk Trip ${p.version} είναι τώρα διαθέσιμη. Επισκεφθείτε τον πίνακα διαχείρισης για να ενημερώσετε.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Γεια σας',
    body: 'Λάβαμε ένα αίτημα επαναφοράς του κωδικού πρόσβασης για τον λογαριασμό σας στο Kuk Trip. Κάντε κλικ στο παρακάτω κουμπί για να ορίσετε νέο κωδικό πρόσβασης.',
