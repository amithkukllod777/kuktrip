...
const admin: TranslationStrings = {
  ...
  'admin.plugins.requiresTrek': '需要 Kuk Trip {version}+',
  ...
  'admin.plugins.reviewedMeaning':
    '“已审核”表示 Kuk Trip 维护者已针对该插件的每个版本进行了恶意软件扫描 —— 而非检查其质量或是否可用。这并不保证插件无害。',
  ...
  'admin.plugins.security.isolationBody':
    '插件作为其独立的受限进程运行，只能读取自己的文件。它无法读取你的数据库、登录密钥或加密密钥，无法启动其他程序，也无法在任何位置写入文件。它的界面运行在一个封闭的浏览器框架中，既读不到你的会话 Cookie，也无法触及周围的 Kuk Trip 页面。',
  'admin.plugins.security.permsTitle': '权限的含义',
  'admin.plugins.security.permsBody':
    '安装前显示的权限是 Kuk Trip 在插件运行期间强制执行的硬性上限 —— 凡是不在清单上的事，插件根本无法做到。但它们只告诉你插件能做什么，而非它实际做了什么。一个获准读取你的行程并连接某服务器的插件，就能把你的行程发送到该服务器，因此请仔细阅读权限和出站主机，而不只是看描述。',
  'admin.plugins.security.limitsTitle': '我们无法承诺的事',
  'admin.plugins.security.limitsBody':
    '这种隔离是强有力的软件边界，但并非绝对。插件所拥有的权限完全就是你批准的那些，因此在这些权限范围内，它的行为可能与其描述不符，也可能把它合法持有的数据发送到它所声明的主机。Kuk Trip 不会读取或评判插件代码实际做了什么。',
  ...
  'admin.plugins.security.reviewedBody':
    '已审核的插件，其每个版本都经过 Kuk Trip 维护者的人工恶意软件扫描 —— 检查的是有无恶意代码，而非它是否运行良好。这并不保证插件无害。',
  'admin.plugins.security.trustTitle': '归根结底',
  'admin.plugins.security.trustBody':
    '安装插件就如同安装任何第三方应用：只添加来自你信任的作者的代码，如有疑虑，请先自行检查。Kuk Trip 对第三方插件不承担任何责任。',
  ...
  'admin.addons.subtitle': '启用或禁用功能以自定义你的 Kuk Trip 体验。',
  ...
  'admin.weather.description': 'Kuk Trip 使用 Open-Meteo 作为天气数据源。Open-Meteo 是免费的开源天气服务——无需 API 密钥。',
  ...
  'admin.github.support': '帮助我继续开发 Kuk Trip',
  'admin.github.error': '加载版本失败',
  'admin.github.by': '作者',
  'admin.update.available': '有可用更新',
  'admin.update.text': 'Kuk Trip {version} 已发布。你当前使用的是 {current}。',
  'admin.update.button': '在 GitHub 查看',
  'admin.update.install': '安装更新',
  'admin.update.confirmTitle': '确定安装更新？',
  'admin.update.confirmText': 'Kuk Trip 将从 {current} 更新到 {version}。服务器将自动重启。',
  ...
  'admin.update.dockerText': '你的 Kuk Trip 实例运行在 Docker 中。要更新到 {version}，请在服务器上执行以下命令：',
  'admin.update.nonDockerText':
    '此 Kuk Trip 实例未运行在 Docker 中。要更新到 {version}，请重新执行你当初使用的安装或更新方式——例如，在 Proxmox Community Scripts 上，从 LXC 控制台运行更新：',
