...
const ja: NotificationLocale = {
  email: {
    footer: 'Kuk Tripで通知を有効にしているため、このメールが届きました。',
    manage: '設定で通知設定を管理',
    madeWith: 'Made with',
    openTrek: 'Kuk Tripを開く',
  },
  events: {
    trip_invite: (p) => ({
      title: `「${p.trip}」への旅行招待`,
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor}が休暇プランの統合に招待しています。Kuk Tripを開いて承認または拒否してください。`,
    }),
    collection_invite: (p) => ({
      title: 'コレクション招待',
      body: `${p.actor}がコレクションの共有に招待しています。Kuk Tripを開いて承認または拒否してください。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新しいKuk Tripバージョンが利用可能',
      body: `Kuk Trip ${p.version}が利用可能になりました。管理パネルからアップデートしてください。`,
    }),
  ...
  passwordReset: {
    subject: 'パスワードをリセット',
    greeting: 'こんにちは',
    body: 'Kuk Tripアカウントのパスワードリセットリクエストを受け付けました。以下のボタンをクリックして新しいパスワードを設定してください。',
    ctaIntro: 'パスワードをリセット',
