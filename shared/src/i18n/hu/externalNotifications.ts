...
const hu: NotificationLocale = {
  email: {
    footer: 'Ezt az értesítést azért kaptad, mert engedélyezted az értesítéseket a Kuk Trip-ben.',
    manage: 'Beállítások kezelése',
    madeWith: 'Made with',
    openTrek: 'Kuk Trip megnyitása',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a Kuk Trip-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    collection_invite: (p) => ({
      title: 'Gyűjtemény meghívó',
      body: `${p.actor} meghívott egy gyűjtemény megosztására. Nyissa meg a Kuk Trip-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Új Kuk Trip verzió érhető el',
      body: `A Kuk Trip ${p.version} elérhető. Látogasson el az adminisztrációs panelre a frissítéshez.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Szia',
    body: 'Kérést kaptunk a Kuk Trip-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.',
