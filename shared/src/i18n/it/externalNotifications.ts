...
const it: NotificationLocale = {
  email: {
    footer: 'Hai ricevuto questa email perché hai le notifiche abilitate in Kuk Trip.',
    manage: 'Gestisci le preferenze nelle impostazioni',
    madeWith: 'Made with',
    openTrek: 'Apri Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri Kuk Trip per accettare o rifiutare.`,
    }),
    collection_invite: (p) => ({
      title: 'Invito a una raccolta',
      body: `${p.actor} ti ha invitato a condividere una raccolta. Apri Kuk Trip per accettare o rifiutare.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nuova versione Kuk Trip disponibile',
      body: `Kuk Trip ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ciao',
    body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account Kuk Trip. Clicca il pulsante qui sotto per impostare una nuova password.',
