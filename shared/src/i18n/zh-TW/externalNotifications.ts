...
const zhTW: NotificationLocale = {
  email: {
    footer: '您收到這封郵件是因為您在 Kuk Trip 中啟用了通知。',
    manage: '管理偏好設定',
    madeWith: 'Made with',
    openTrek: '開啟 Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
      title: `邀請加入「${p.trip}」`,
    ...
    vacay_invite: (p) => ({
      title: 'Vacay 融合邀請',
      body: `${p.actor} 邀請您合併假期計畫。開啟 Kuk Trip 以接受或拒絕。`,
    }),
    collection_invite: (p) => ({
      title: '收藏邀請',
      body: `${p.actor} 邀請您共享收藏。開啟 Kuk Trip 以接受或拒絕。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新版 Kuk Trip 可用',
      body: `Kuk Trip ${p.version} 現已可用。請前往管理面板進行更新。`,
    }),
  ...
  passwordReset: {
    subject: '重設您的密碼',
    greeting: '您好',
    body: '我們收到了重設您 Kuk Trip 帳號密碼的請求。點擊下方按鈕以設定新密碼。',
    ctaIntro: '重設密碼',
