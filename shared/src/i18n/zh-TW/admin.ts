...
const admin: TranslationStrings = {
  ...
  'admin.plugins.reviewedMeaning':
    '「已審核」代表 Kuk Trip 維護者曾針對每個版本掃描此外掛是否含有惡意程式碼——並非檢查品質或功能是否正常。這不保證外掛無害。',
  ...
  'admin.plugins.security.isolationBody':
    '外掛以自身受限的獨立程序執行，只能讀取自己的檔案。它無法讀取你的資料庫、登入密鑰或加密金鑰，無法啟動其他程式，也無法在任何位置寫入檔案。它的介面在一個封閉的瀏覽器框架中執行，無法讀取你的工作階段 Cookie，也無法觸及其周圍的 Kuk Trip 頁面。',
  'admin.plugins.security.permsTitle': '這些權限的含義',
  'admin.plugins.security.permsBody':
    '安裝前顯示的權限是 Kuk Trip 在外掛執行期間強制執行的硬性上限——它絕對無法做出清單以外的任何動作。但這些權限告訴你外掛「能」做什麼，而非它「實際」做什麼。一個獲准讀取你行程並連線某伺服器的外掛，就能將你的行程傳送到該伺服器，所以請閱讀權限與對外主機，而不只是看說明。',
  'admin.plugins.security.limitsTitle': '我們無法保證的事',
  'admin.plugins.security.limitsBody':
    '這種隔離是一道強而有力的軟體邊界，但並非絕對。外掛僅以你核准的權限行事，因此在這些權限範圍內，它的行為可能與其說明不符，也可能將它合法持有的資料傳送到它所宣告的主機。Kuk Trip 不會讀取或評判外掛的程式碼實際做了什麼。',
  ...
  'admin.plugins.security.reviewedBody':
    '已審核的外掛，代表 Kuk Trip 維護者曾針對每個版本手動掃描惡意程式碼——檢查是否含有有害程式碼，而非它是否運作良好。這並不保證外掛無害。',
  'admin.plugins.security.trustTitle': '結論',
  'admin.plugins.security.trustBody':
    '安裝外掛就像安裝任何第三方應用程式：只加入你信任的作者所提供的程式碼，如有疑慮，請先自行檢查。Kuk Trip 對第三方外掛概不承擔任何責任。',
  ...
  'admin.addons.subtitle': '啟用或停用功能以自定義你的 Kuk Trip 體驗。',
  ...
  'admin.weather.description':
    'Kuk Trip 使用 Open-Meteo 作為天氣資料來源。Open-Meteo 是免費的開源天氣服務——無需 API 金鑰。',
  ...
  'admin.github.support': '幫助我繼續開發 Kuk Trip',
  'admin.github.error': '載入版本失敗',
  'admin.github.by': '作者',
  'admin.update.available': '有可用更新',
  'admin.update.text': 'Kuk Trip {version} 已釋出。你當前使用的是 {current}。',
  'admin.update.button': '在 GitHub 檢視',
  'admin.update.install': '安裝更新',
  'admin.update.confirmTitle': '確定安裝更新？',
  'admin.update.confirmText': 'Kuk Trip 將從 {current} 更新到 {version}。伺服器將自動重啟。',
  ...
  'admin.update.dockerText': '你的 Kuk Trip 例項執行在 Docker 中。要更新到 {version}，請在伺服器上執行以下命令：',
  'admin.update.nonDockerText':
    '此 Kuk Trip 例項並非執行在 Docker 中。要更新到 {version}，請重新執行你當初使用的安裝或更新方式——例如在 Proxmox Community Scripts 上，請從 LXC 主控臺執行更新：',
