...
const en: NotificationLocale = {
  email: {
    footer: 'Du har fått detta eftersom du har aktiverat aviseringar i Kuk Trip.',
    manage: 'Hantera egenskaper under Inställningar',
    madeWith: 'Gjorde med',
    openTrek: 'Öppna Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} bjöd in dig att slå samman semesterplaner. Öppna Kuk Trip för att acceptera eller avvisa.`,
    }),
    collection_invite: (p) => ({
      title: 'Inbjudan till samling',
      body: `${p.actor} bjöd in dig att dela en samling. Öppna Kuk Trip för att acceptera eller avvisa.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Ny Kuk Trip version tillgänglig',
      body: `Kuk Trip ${p.version} är nu tillgänglig. Gå till adminpanelen för att uppdatera.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hej',
    body: 'Vi har fått en begäran om att återställa lösenordet till ditt Kuk Trip konto. Klicka på knappen nedan för att ange ett nytt lösenord.',
