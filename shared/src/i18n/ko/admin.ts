...
const admin: TranslationStrings = {
  ...
  'admin.plugins.requiresTrek': 'Kuk Trip {version} 이상 필요',
  ...
  'admin.plugins.reviewedMeaning':
    '"검토됨"은 Kuk Trip 관리자가 각 버전마다 이 플러그인에 악성코드가 있는지 검사했다는 의미입니다 — 품질이나 정상 작동 여부를 검사한 것이 아닙니다. 플러그인이 무해하다는 보장은 아닙니다.',
  ...
  'admin.plugins.security.isolationBody':
    '플러그인은 자체 파일만 읽을 수 있는 잠긴 별도 프로세스로 실행됩니다. 데이터베이스, 로그인 비밀 값, 암호화 키를 읽을 수 없고, 다른 프로그램을 실행할 수 없으며, 어디에도 파일을 쓸 수 없습니다. 플러그인의 인터페이스는 세션 쿠키를 읽거나 주변 Kuk Trip 페이지를 건드릴 수 없는 봉인된 브라우저 프레임에서 실행됩니다.',
  'admin.plugins.security.permsTitle': '권한이 의미하는 것',
  'admin.plugins.security.permsBody':
    '설치 전에 표시되는 권한은 플러그인이 실행되는 동안 Kuk Trip이 강제하는 엄격한 한계입니다 — 목록에 없는 것은 말 그대로 아무것도 할 수 없습니다. 하지만 이는 플러그인이 무엇을 할 수 있는지를 알려줄 뿐, 실제로 무엇을 하는지는 알려주지 않습니다. 여행을 읽고 서버에 연결할 수 있는 플러그인은 그 서버로 여행 데이터를 전송할 수 있으므로, 설명뿐만 아니라 권한과 아웃바운드 호스트도 확인하세요.',
  'admin.plugins.security.limitsTitle': '보장할 수 없는 것',
  'admin.plugins.security.limitsBody':
    '이 격리는 강력한 소프트웨어 경계이지만 절대적인 것은 아닙니다. 플러그인은 정확히 승인한 권한만큼 동작하므로, 그 권한 범위 내에서는 설명과 다르게 동작할 수 있으며, 정당하게 보유한 데이터를 선언한 호스트로 전송할 수 있습니다. Kuk Trip은 플러그인의 코드가 실제로 무엇을 하는지 읽거나 판단하지 않습니다.',
  ...
  'admin.plugins.security.reviewedBody':
    '검토된 플러그인은 각 버전마다 Kuk Trip 관리자가 악성코드를 수동으로 검사한 것입니다 — 정상 작동 여부가 아니라 악의적인 코드가 있는지 확인한 것입니다. 플러그인이 무해하다는 약속은 아닙니다.',
  'admin.plugins.security.trustTitle': '핵심 요약',
  'admin.plugins.security.trustBody':
    '플러그인을 설치하는 것은 다른 서드파티 앱을 설치하는 것과 같습니다: 신뢰할 수 있는 작성자의 코드만 추가하고, 의심스러우면 먼저 직접 확인하세요. Kuk Trip은 서드파티 플러그인에 대해 어떠한 책임도 지지 않습니다.',
  ...
  'admin.addons.subtitle': '기능을 활성화 또는 비활성화하여 Kuk Trip 경험을 맞춤 설정하세요.',
  ...
  'admin.weather.description':
    'Kuk Trip은 날씨 데이터 소스로 Open-Meteo를 사용합니다. Open-Meteo는 무료 오픈 소스 날씨 서비스로 API 키가 필요 없습니다.',
  ...
  'admin.github.by': '작성자',
  'admin.github.support': 'Kuk Trip 개발 지속에 도움이 됩니다',
  'admin.update.available': '업데이트 사용 가능',
  'admin.update.text': 'Kuk Trip {version}이(가) 사용 가능합니다. 현재 {current}을(를) 실행 중입니다.',
  'admin.update.button': 'GitHub에서 보기',
  'admin.update.install': '업데이트 설치',
  'admin.update.confirmTitle': '업데이트를 설치할까요?',
  'admin.update.confirmText': 'Kuk Trip이 {current}에서 {version}으로 업데이트됩니다. 서버가 이후 자동으로 재시작됩니다.',
  ...
  'admin.update.dockerText':
    'Kuk Trip 인스턴스가 Docker에서 실행 중입니다. {version}으로 업데이트하려면 서버에서 다음 명령을 실행하세요:',
  'admin.update.nonDockerText':
    '이 Kuk Trip 인스턴스는 Docker에서 실행되고 있지 않습니다. {version}으로 업데이트하려면 사용했던 설치 또는 업데이트 방법을 다시 실행하세요 — 예를 들어 Proxmox Community Scripts에서는 LXC 콘솔에서 업데이트를 실행하세요:',
