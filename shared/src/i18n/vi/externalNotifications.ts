...
const en: NotificationLocale = {
  email: {
    footer: 'Bạn nhận được thông báo này vì bạn đã bật thông báo trong Kuk Trip.',
    manage: 'Quản lý tùy chọn trong Cài đặt',
    madeWith: 'Được làm bằng',
    openTrek: 'Mở Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} đã mời bạn kết hợp kế hoạch kỳ nghỉ. Mở Kuk Trip để chấp nhận hoặc từ chối.`,
    }),
    collection_invite: (p) => ({
      title: 'Lời mời bộ sưu tập',
      body: `${p.actor} đã mời bạn chia sẻ một bộ sưu tập. Mở Kuk Trip để chấp nhận hoặc từ chối.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Đã có phiên bản Kuk Trip mới',
      body: `Kuk Trip ${p.version} có bản mới. Vui lòng truy cập bảng điều khiển quản trị để cập nhật.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'CHÀO',
    body: 'Chúng tôi đã nhận được yêu cầu đặt lại mật khẩu cho tài khoản Kuk Trip của bạn. Nhấp vào nút bên dưới để đặt mật khẩu mới.',
