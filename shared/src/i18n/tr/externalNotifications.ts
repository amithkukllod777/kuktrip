...
const tr: NotificationLocale = {
  email: {
    footer: "Kuk Trip'te bildirimleri etkinleştirdiğiniz için bunu aldınız.",
    manage: 'Ayarlarda tercihleri yönetin',
    madeWith: 'Made with',
    openTrek: "Kuk Trip'i aç",
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} sizi tatil planlarını birleştirmeye davet etti. Kabul etmek veya reddetmek için Kuk Trip'i açın.`,
    }),
    collection_invite: (p) => ({
      title: 'Koleksiyon daveti',
      body: `${p.actor} sizi bir koleksiyonu paylaşmaya davet etti. Kabul etmek veya reddetmek için Kuk Trip’i açın.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Yeni Kuk Trip sürümü mevcut',
      body: `Kuk Trip ${p.version} artık mevcut. Güncellemek için yönetici panelini ziyaret edin.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Merhaba',
    body: 'Kuk Trip hesabınızın şifresini sıfırlamak için bir istek aldık. Yeni bir şifre belirlemek için aşağıdaki butona tıklayın.',
