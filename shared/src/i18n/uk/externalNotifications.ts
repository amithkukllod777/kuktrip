...
const uk: NotificationLocale = {
  email: {
    footer: 'Ви отримали це, оскільки увімкнули сповіщення в Kuk Trip.',
    manage: 'Керувати налаштуваннями у Налаштуваннях',
    madeWith: 'Made with',
    openTrek: 'Відкрити Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} запрошує вас об'єднати плани відпустки. Відкрийте Kuk Trip, щоб прийняти або відхилити.`,
    }),
    collection_invite: (p) => ({
      title: 'Запрошення до колекції',
      body: `${p.actor} запрошує вас поділитися колекцією. Відкрийте Kuk Trip, щоб прийняти або відхилити.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Доступна нова версія Kuk Trip',
      body: `Kuk Trip ${p.version} тепер доступний. Перейдіть до панелі адміністратора для оновлення.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Привіт',
    body: 'Ми отримали запит на скидання пароля вашого облікового запису Kuk Trip. Натисніть кнопку нижче, щоб встановити новий пароль.',
