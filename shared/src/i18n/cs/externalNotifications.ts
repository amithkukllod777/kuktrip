...
const cs: NotificationLocale = {
  email: {
    footer: 'Toto jsi obdržel/a, protože máš povoleny upozornění v Kuk Trip.',
    manage: 'Spravovat předvolby v nastavení',
    madeWith: 'Made with',
    openTrek: 'Otevřít Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete Kuk Trip pro přijetí nebo odmítnutí.`,
    }),
    collection_invite: (p) => ({
      title: 'Pozvánka do sbírky',
      body: `${p.actor} vás pozval ke sdílení sbírky. Otevřete Kuk Trip pro přijetí nebo odmítnutí.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nová verze Kuk Trip dostupná',
      body: `Kuk Trip ${p.version} je nyní dostupný. Navštivte administrátorský panel pro aktualizaci.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ahoj',
    body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu Kuk Trip. Klikni na tlačítko níže a nastav nové heslo.',
