...
const ru: NotificationLocale = {
  email: {
    footer: 'Вы получили это, потому что у вас включены уведомления в Kuk Trip.',
    manage: 'Управление настройками',
    madeWith: 'Made with',
    openTrek: 'Открыть Kuk Trip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} приглашает вас объединить планы отпуска. Откройте Kuk Trip для подтверждения.`,
    }),
    collection_invite: (p) => ({
      title: 'Приглашение в коллекцию',
      body: `${p.actor} приглашает вас поделиться коллекцией. Откройте Kuk Trip для подтверждения.`,
    }),
    photos_shared: (p) => ({
      title: `${p.count} фото`,
    ...
    version_available: (p) => ({
      title: 'Доступна новая версия Kuk Trip',
      body: `Kuk Trip ${p.version} теперь доступен. Перейдите в панель администратора для обновления.`,
    }),
  ...
  passwordReset: {
    ...
    body: 'Мы получили запрос на сброс пароля вашего аккаунта Kuk Trip. Нажмите кнопку ниже, чтобы установить новый пароль.',
