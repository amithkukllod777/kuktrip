...
class TripRepository(context: Context) {
    ...
    suspend fun listTrips(archived: Boolean = false): List<Trip> = withContext(Dispatchers.IO) {
        val response = api.listTrips(bearer(), if (archived) 1 else 0)
        if (!response.isSuccessful) throw TripDataException("Could not load trips (${response.code()}).")
        response.body()?.trips.orEmpty()
    }

    suspend fun getTrip(id: Int): Trip = withContext(Dispatchers.IO) {
        val response = api.getTrip(bearer(), id)
        if (!response.isSuccessful) throw TripDataException("Could not load this trip (${response.code()}).")
        response.body()?.trip ?: throw TripDataException("Trip response was empty.")
    }

    suspend fun createTrip(body: CreateTripBody): Trip = withContext(Dispatchers.IO) {
        val response = api.createTrip(bearer(), body)
        if (!response.isSuccessful) throw TripDataException("Could not create trip (${response.code()}).")
        response.body()?.trip ?: throw TripDataException("Trip response was empty.")
    }

    suspend fun deleteTrip(id: Int) = withContext(Dispatchers.IO) {
        val response = api.deleteTrip(bearer(), id)
        if (!response.isSuccessful) throw TripDataException("Could not delete trip (${response.code()}).")
    }

    suspend fun listDays(tripId: Int): List<Day> = withContext(Dispatchers.IO) {
        val response = api.listDays(bearer(), tripId)
        if (!response.isSuccessful) throw TripDataException("Could not load itinerary (${response.code()}).")
    ...
    suspend fun addDay(tripId: Int): Day = withContext(Dispatchers.IO) {
        ...
        response.body()?.day ?: throw TripDataException("Day response was empty.")
    }
}
